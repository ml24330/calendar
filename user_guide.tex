...
\begin{itemize}
  ...
  \item \textbf{Duplicate} — opens a new event with every field copied from
        the one you are looking at. Change the date and time and save; the
        original is left as it was. Handy for events that repeat.
...
\subsection{Published and unpublished events}

The event form has two save buttons. \textbf{Add and publish} makes the event
visible to everyone. \textbf{Add as unpublished} keeps it visible only to people
logged in to edit — it stays out of the feed, the downloads and the PDF.

Leave an event unpublished when a date or venue is not confirmed. Unpublished
events appear hatched and
...
every event including unpublished ones in a format any calendar can read.
...
\begin{itemize}
  ...
  \item \textbf{A feed link containing a token shows unpublished events.} Calendar apps
